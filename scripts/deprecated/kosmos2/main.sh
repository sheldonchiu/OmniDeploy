...
if [[ "$REINSTALL_KOSMOS2" || ! -f "/tmp/kosmos2.prepared" ]]; then
    ...
    $UV_INSTALL_DIR/uv venv --seed --python 3.10 $VENV_DIR/kosmos2-env
    
    source $VENV_DIR/kosmos2-env/bin/activate
    
    cd $REPO_DIR/kosmos-2

    $UV_INSTALL_DIR/uv pip install torch==1.13.1+cu116 torchvision==0.14.1+cu116 torchaudio==0.13.1 --extra-index-url https://download.pytorch.org/whl/cu116
    $UV_INSTALL_DIR/uv pip install xformers==0.0.16
    $UV_INSTALL_DIR/uv pip install protobuf==3.20.3 pytorch-extension

    $UV_INSTALL_DIR/uv pip install fairseq/
    $UV_INSTALL_DIR/uv pip install infinibatch/
    $UV_INSTALL_DIR/uv pip install torchscale/
    $UV_INSTALL_DIR/uv pip install open_clip/
    $UV_INSTALL_DIR/uv pip install  git+https://github.com/microsoft/DeepSpeed.git@jeffra/engine-xthru-v2
    $UV_INSTALL_DIR/uv pip install numpy==1.23.0 scipy==1.8.0 tiktoken \
    ...
    case $gpu_name in
      *A4000*)
        $UV_INSTALL_DIR/uv pip install -v --disable-$UV_INSTALL_DIR/uv pip-version-check --no-cache-dir --no-build-isolation  .
      ...
      *)
        ...
        $UV_INSTALL_DIR/uv pip install -v --disable-$UV_INSTALL_DIR/uv pip-version-check --no-cache-dir --no-build-isolation --config-settings "--build-option=--cpp_ext" --config-settings "--build-option=--cuda_ext" ./
        ;;
    esac
    cd $REPO_DIR
